            ...
            for (const c of countries) {
                ...
                <div class="card" style="background:white;border-radius:12px;padding:16px;margin-bottom:16px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
                    ...
                    <div class="all-links-container" id="links-${index}" style="display:none;margin-top:10px;padding:10px;background:#f8fafc;border-radius:8px;">
                        <div class="links-stats" style="display:flex;justify-content:space-between;margin-bottom:8px;"><span>📌 ${links.length} رابط بحث متقدم</span><span style="font-size:12px;color:#94a3b8;">للبحث عن: ${escapeHtml(query)}</span></div>
                        <div class="links-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:6px;">${links.map(link => `
            ...
            for (const c of cities) {
                ...
                <div class="card" style="background:white;border-radius:12px;padding:16px;margin-bottom:16px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
                    ...
                    <div class="seo-tags" style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px;">
                        ...
                        <a class="btn btn-google" target="_blank" rel="noopener noreferrer" href="https://www.google.com/search?q=${encodeURIComponent('مطاعم ' + city + ' ' + country)}" style="padding:4px 10px;background:#eef2ff;color:#4338ca;border-radius:8px;text-decoration:none;font-size:12px;">🍽️ مطاعم</a>
                    ...
                    <div class="card no-results">
                        <div style="text-align:center;padding:40px;">
                            ...
                            <div style="margin-top:16px;display:flex;gap:10px;justify-content:center;flex-wrap:wrap;">
                                ...
                                <button onclick="searchAllWikipedia()" style="padding:10px 24px;background:#8b5cf6;color:white;border:none;border-radius:8px;cursor:pointer;font-size:16px;">🔍 بحث موسع</button>
                            </div>
                        </div>
                    </div>
                `;
        ...
        html = `
            <div style="margin: 16px 0 8px; padding: 8px 16px; background: #f1f5f9; border-radius: 12px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;">
                ...
                <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                    <button onclick="loadMoreWikipedia()" style="padding: 6px 16px; background: #3b82f6; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 13px;">📚 تحميل المزيد</button>
                    <button onclick="searchAllWikipedia()" style="padding: 6px 16px; background: #8b5cf6; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 13px;">🔍 بحث موسع</button>
                </div>
            </div>
        `;
    }
    results.forEach(item => {
        ...
        html += `
            <div class="card" style="background:white;border-radius:12px;padding:16px;margin-bottom:16px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
                ...
                <div class="btn-group" style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px;">
                    <a class="btn btn-maps" target="_blank" rel="noopener noreferrer" href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(searchQuery)}" style="padding:6px 12px;background:#10b981;color:white;border-radius:8px;text-decoration:none;">📍 خرائط</a>
                    <a class="btn btn-google" target="_blank" rel="noopener noreferrer" href="https://www.google.com/search?q=${encodeURIComponent(searchQuery)}" style="padding:6px 12px;background:#4285f4;color:white;border-radius:8px;text-decoration:none;">🔍 Google</a>
                    <a class="btn btn-yt" target="_blank" rel="noopener noreferrer" href="https://www.youtube.com/results?search_query=${encodeURIComponent(searchQuery)}" style="padding:6px 12px;background:#ff0000;color:white;border-radius:8px;text-decoration:none;">▶ YouTube</a>
                    <a class="btn btn-wiki" target="_blank" rel="noopener noreferrer" href="${item.url}" style="padding:6px 12px;background:#3b82f6;color:white;border-radius:8px;text-decoration:none;">📖 Wiki</a>
                    <button class="btn btn-secondary" onclick="toggleLinks(${index})" style="padding:6px 12px;background:#f1f5f9;border:none;border-radius:8px;cursor:pointer;font-size:13px;">📋 عرض الروابط (${links.length})</button>
                    <button class="btn btn-favorite favorite-toggle" data-favorite-index="${index}" onclick="toggleFavorite(${index})" type="button" aria-label="إضافة ${escapeHtml(searchQuery)} إلى المفضلة" style="padding:6px 12px;background:#f1f5f9;border:none;border-radius:8px;cursor:pointer;font-size:13px;">☆ مفضلة</button>
                </div>
                <div class="seo-tags" style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px;">
                    <a class="btn btn-google" target="_blank" rel="noopener noreferrer" href="https://www.google.com/search?q=${encodeURIComponent('تاريخ ' + searchQuery)}" style="padding:4px 10px;background:#eef2ff;color:#4338ca;border-radius:8px;text-decoration:none;font-size:12px;">📜 تاريخ</a>
                    <a class="btn btn-google" target="_blank" rel="noopener noreferrer" href="https://www.google.com/search?q=${encodeURIComponent('معلومات عن ' + searchQuery)}" style="padding:4px 10px;background:#eef2ff;color:#4338ca;border-radius:8px;text-decoration:none;font-size:12px;">ℹ️ معلومات</a>
                    <a class="btn btn-google" target="_blank" rel="noopener noreferrer" href="https://www.google.com/search?q=${encodeURIComponent('صور ' + searchQuery)}" style="padding:4px 10px;background:#eef2ff;color:#4338ca;border-radius:8px;text-decoration:none;font-size:12px;">📷 صور</a>
                    <a class="btn btn-google" target="_blank" rel="noopener noreferrer" href="https://www.google.com/search?q=${encodeURIComponent('فيديو ' + searchQuery)}" style="padding:4px 10px;background:#eef2ff;color:#4338ca;border-radius:8px;text-decoration:none;font-size:12px;">🎬 فيديو</a>
                    <a class="btn btn-google" target="_blank" rel="noopener noreferrer" href="https://en.wikipedia.org/wiki/${encodeURIComponent(searchQuery)}" style="padding:4px 10px;background:#eef2ff;color:#4338ca;border-radius:8px;text-decoration:none;font-size:12px;">🌐 EN Wiki</a>
                </div>
                <div class="all-links-container" id="links-${index}" style="display:none;margin-top:10px;padding:10px;background:#f8fafc;border-radius:8px;">
                    <div class="links-stats" style="display:flex;justify-content:space-between;margin-bottom:8px;"><span>📌 ${links.length} رابط بحث متقدم</span><span style="font-size:12px;color:#94a3b8;">للبحث عن: ${escapeHtml(searchQuery)}</span></div>
                    <div class="links-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:6px;">${links.map(link => `
                        <a class="link-item" target="_blank" rel="noopener noreferrer" href="${link.url}" style="padding:4px 8px;background:white;border-radius:4px;text-decoration:none;color:inherit;">
                            <span class="link-number" style="color:#94a3b8;">#${link.id}</span>
                            <span class="link-name" style="margin-right:4px;">${escapeHtml(link.name)}</span>
    ...
    if (!isMore) {
        html += `
            <div style="text-align:center;margin:20px 0;padding:15px;background:#f8fafc;border-radius:12px;">
                <button onclick="loadMoreWikipedia()" style="padding:12px 30px;background:#3b82f6;color:white;border:none;border-radius:8px;cursor:pointer;font-size:15px;">📚 تحميل المزيد من نتائج ويكيبيديا</button>
                <button onclick="searchAllWikipedia()" style="margin-right:10px;padding:12px 30px;background:#8b5cf6;color:white;border:none;border-radius:8px;cursor:pointer;font-size:15px;">🔍 بحث موسع (100 نتيجة)</button>
            </div>
        `;
    }
    resultsDiv.innerHTML += html;
