...
function buttonsHtml(query,index,mapsQuery=query,wikiUrl=''){
    ...
    const mapsText=String(mapsQuery||text);
    ...
    return `<div class="btn-group" style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px;">
        ...
        <a class="btn btn-secondary advanced-category-link" data-query="${escapeHtml(text)}" target="_blank" rel="noopener noreferrer" href="${escapeHtml(advancedUrl)}"><span class="advanced-category-link-text">⚡ ${escapeHtml(getCategoryName(selectedAdvancedCategory))}</span></a>
    </div>`;
}
